...
- Styles are SCSS modules (`*.module.scss`) co-located with the component or page that owns them (`components/SiteHeader.module.scss` next to `components/SiteHeader.tsx`; `app/houses/[slug]/page.module.scss` next to its `page.tsx`). SCSS modules are compiled by Next.js's built-in Sass support (the `sass` devDependency). The only global stylesheet is `styles/globals.scss` (resets, CSS custom properties, `html`/`body`/`h1-h3` rules, and the `.subtitle` typographic primitive). No Tailwind, no CSS-in-JS: preserve the parchment aesthetic.
...
- All in-repo imports use the `@/` alias (`@/components/Sigil`, `@/lib/schemas`, `@/components/SiteHeader.module.scss`) — never relative paths like `./Foo` or `../package.json`. Applies to source, tests, and SCSS-module imports alike. Third-party imports (`next/link`, `react`, etc.) stay bare.
