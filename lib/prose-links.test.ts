...
function house(partial: Partial<House> & Pick<House, 'slug' | 'name'>): HouseFixture {
  const fm: House = {
    slug: partial.slug,
    name: partial.name,
    seat: partial.seat ?? '',
...
const RHAEGAR = character({ slug: 'rhaegar-targaryen', name: 'Rhaegar Targaryen' });
...
async function renderWith(source: string, index: ProseLinkIndex): Promise<string> {
  return renderMarkdown(source, { proseLinks: index });
}

describe('prose-links', () => {
  it('links a full character name in prose', async () => {
    const index = indexFor({ current: { kind: 'character', slug: 'lyanna-stark' } });
    const html = await renderWith('Betrothed to Catelyn Tully of Riverrun.', index);
    expect(html).toContain('<a href="/characters/catelyn-tully/">Catelyn Tully</a>');
  });

  it('links an alias inside emphasis', async () => {
    const index = indexFor({ current: { kind: 'character', slug: 'eddard-stark' } });
    const html = await renderWith('Called *the Wild Wolf* for his temper.', index);
    expect(html).toContain('<a href="/characters/brandon-stark/">the Wild Wolf</a>');
  ...
  it('first-name matches when slug is listed in `mentions`', async () => {
    const index = indexFor({
      current: { kind: 'character', slug: 'eddard-stark', mentions: ['rickard-stark'] },
    });
    const html = await renderWith('Lord Rickard was burned alive.', index);
  ...
  it('matches longest surface form when forms overlap', async () => {
    ...
    expect(html).toContain('<a href="/characters/brandon-stark/">Brandon Stark</a>');
  ...
  it('suppresses self-links', async () => {
    const index = indexFor({ current: { kind: 'character', slug: 'brandon-stark' } });
  ...
  it('skips matches inside code spans', async () => {
    const index = indexFor({ current: { kind: 'character', slug: 'eddard-stark' } });
    const html = await renderWith('The variable `Catelyn Tully` is reserved.', index);
  ...
  it('matches an alias like "Aerys II" as full surface form', async () => {
    ...
    expect(html).toContain('<a href="/characters/aerys-ii-targaryen/">Aerys II</a>');
  });
  ...
  it('links Brandon’s example paragraph end-to-end', async () => {
    ...
    expect(html).toContain('<a href="/characters/rickard-stark/">Rickard</a>');
    expect(html).toContain('<a href="/characters/catelyn-tully/">Catelyn Tully</a>');
    expect(html).toContain('<a href="/characters/rhaegar-targaryen/">Rhaegar</a>');
  ...
  it('skips draft and placeholder characters', async () => {
    const draftChar = character({ slug: 'ghost-author', name: 'Ghost Author', draft: true });
    ...
    expect(html).toContain('<a href="/characters/rickard-stark/">Rickard Stark</a>');
