...
Sigils are deliberately out of scope here. `lib/sigil-integrity.ts` owns that resolution order and is gated in CI, so use `sigil-audit` for them. Auditing them in two places would fork the rules.
...
```bash
bun .claude/skills/image-optimize/audit-assets.ts
bun .claude/skills/image-optimize/audit-assets.ts --json
```
...
**Needs a code change, and read `sigil-audit` first.** `Sigil.tsx` hardcodes the extension:
...
```bash
bun .claude/skills/image-optimize/audit-assets.ts   # broken refs back to none
