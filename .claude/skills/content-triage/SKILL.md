...
`content/characters/` holds 920 entries and a large minority are unwritten. The hard part is not writing one entry, `populate-character` already does that. The hard part is that a raw count of unwritten entries is misleading in two directions at once, so picking targets by eye picks wrong.
...
```bash
bun .claude/skills/content-triage/triage.ts              # top 25
bun .claude/skills/content-triage/triage.ts --limit 40
bun .claude/skills/content-triage/triage.ts --state empty  # only zero-body entries
bun .claude/skills/content-triage/triage.ts --json         # machine-readable
```
...
2. Populate each with `populate-character`.
