...
Two populate skills already cover 1,388 of the repo's 1,696 content entries: `tkw-populate-character` (920) and `tkw-populate-house` (468). The remaining five collections hold 308 entries and had no skill at all.
...
bun .claude/skills/tkw-populate-entry/audit-entries.ts --limit 40 --json
```
...
```bash
bun .claude/skills/tkw-populate-entry/audit-entries.ts                       # rank all five collections
bun .claude/skills/tkw-populate-entry/audit-entries.ts --collection battles  # one collection
bun .claude/skills/tkw-populate-entry/audit-entries.ts --limit 40 --json     # machine readable
...
- `tkw-populate-character`, `tkw-populate-house`: the other two collections, same AWOIAF pipeline
- `tkw-content-triage`: ranks the 920 character entries, which this audit deliberately excludes
- `tkw-commit-format`, `tkw-pr-format`: the `TKW:` conventions for shipping the work
