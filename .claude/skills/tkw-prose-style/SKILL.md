...
- `tkw-populate-character` and `tkw-populate-house`: write the prose in the first place; both should be read alongside the voice table above
- `tkw-content-triage`: picks which entries to write next
- `tkw-sigil-audit`: the same read-only, report-and-stop shape applied to art wiring
