...
bun .claude/skills/map-placement/audit-placement.ts

# Machine-readable, for batching
bun .claude/skills/map-placement/audit-placement.ts --json
...
- `content-triage`: ranks which entries are worth populating before they are worth placing
- `image-optimize`: owns `public/map/` asset weight, including the 11 MB raster
