...
bun .claude/skills/tkw-component-scaffold/audit-components.ts --json
...
- `tkw-sigil-audit`: the other structural audit in this repo, for sigil wiring
- `tkw-image-optimize`: `public/` asset audit, same read-only shape
- `tkw-commit-format` and `tkw-pr-format`: the `TKW:` conventions for shipping the fix
