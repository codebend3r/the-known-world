...
## Boundary with `content-triage`

Both count inbound references; they rank on opposite axes.

| Skill            | Ranks by                  | Its finding                                 |
| ---------------- | ------------------------- | ------------------------------------------- |
| `content-triage` | how **empty** an entry is | a thin stub that lots of pages already want |
| `orphan-content` | how **unreachable** it is | a finished entry that no page links to      |
...
```bash
bun .claude/skills/orphan-content/audit-orphans.ts                      # full report
bun .claude/skills/orphan-content/audit-orphans.ts --collection castles # one collection
bun .claude/skills/orphan-content/audit-orphans.ts --json               # machine-readable
...
Measured on `main`, then after the wiring in `docs/superpowers/baselines/orphan-content.md`.
...
When you do need to check AWOIAF, direct `WebFetch` is Cloudflare-blocked and 403s. Use the Wayback CDX API plus `curl` plus `textutil`; `populate-character` documents that pipeline.
...
- `content-triage`: which thin entries to write next, ranked by demand
- `populate-character`, `populate-house`: filling an entry once you have picked it
- `sigil-audit`: the same drift problem for `public/sigils/` and the `SIGIL_SLUGS` allowlist
