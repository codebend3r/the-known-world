...
Fix both with a `(max-width: 639.98px)` media query in the existing stylesheets. No JSX or test changes are needed, since the DOM order is already correct for what we want to express with grid.
...
- ❌ Adjusting the houses list, which is out of scope for this pass.
...
- Letter-spacing on `.character-list__name` is currently `2px`, which is fine on desktop and slightly tight against mobile text. Leave it; revisit if a future pass introduces small-screen typography tweaks elsewhere.
...
- Existing Vitest suite (`FilteredCharacterList.test.tsx`) must continue to pass with no changes, since the DOM structure is preserved.
- Manual verification at 375 px and 320 px widths (Safari/Chrome responsive mode):
  ...
  - A character with no alias (e.g., Arya Stark) centers vertically alongside the portrait, with no awkward top-alignment.
...
- **`align-self: stretch` on the portrait** assumes the image's intrinsic aspect ratio still fits the 5.75 rem cell height, the same assumption as desktop. If a tall portrait ever shipped, the grid row would stretch and so would desktop already, so no new risk.
