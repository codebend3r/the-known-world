# tkw-new-content-type baseline

First run of `bun .claude/skills/tkw-new-content-type/audit-wiring.ts` against `origin/main` (`d11fcfd`), and the fix that followed. The events detail route landed on 2026-07-04 in `4b6eb1c`; the gap survived every commit since.
...
| `components/SiteMenu/SiteMenu.tsx` points `/castles/` at `/menu-icons/castles.png`, which does not exist | Harmless today: castles also supplies a `glyph`, and `art?.glyph ?? <Image>` means the PNG is never requested. It is a dangling reference for `tkw-image-optimize` to sweep, not a wiring gap. |
