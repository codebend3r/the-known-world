...
First run of `tkw-portrait-pipeline` against `main` at `d11fcfd`, plus the fixes it
...
| Converting portraits to WebP                           | `PORTRAIT_EXTENSIONS` already accepts `webp`, so it would work, but every portrait is served through the Netlify Image CDN and re-encoded per request. Repo weight only. Belongs to `tkw-image-optimize` if it is ever done in bulk. |
| `public/sigils/`, `public/map/`, the rest of `public/` | Owned by `tkw-sigil-audit` and `tkw-image-optimize`. Auditing them here would fork the rules.                                                                                                                                        |
