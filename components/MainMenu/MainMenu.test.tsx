...
describe("MainMenu", () => {
  it("renders only visible tiles in order: Maps, Timeline, Houses, Castles, Characters, Weapons, Battles", () => {
    ...
    expect(links[3].textContent).toContain("Castles");
    expect(links[3].getAttribute("href")).toBe("/castles/");

    expect(links[4].textContent).toContain("Characters");
    expect(links[4].getAttribute("href")).toBe("/characters/");
